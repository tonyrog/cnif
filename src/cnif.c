...
static inline ERL_NIF_TERM copy(ErlNifEnv* dst_env, ERL_NIF_TERM src_term)
{
    switch(src_term & 3) {
				   ...
				   ERL_NIF_TERM* srcp)
{
    ...
    dst_mp->keys = copy(dst_env, src_mp->keys);
    for (i = 0; i < size; i++)
	dst_mp->value[i] = copy(dst_env, src_mp->value[i]);
				       ...
				       ERL_NIF_TERM* srcp, ERL_NIF_UINT n)
{
    ...
    dst_sbp->orig      = copy(dst_env, src_sbp->orig);
    return MAKE_BINARY(dstp);
}
